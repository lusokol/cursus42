...
/*   Updated: 2020/03/11 11:42:21 by lusokol          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	all->draw.jump = ((double)all->spr.spriteheight / 3) * (all->coord.z * 10);
	all->spr.drawstarty = -all->spr.spriteheight * (0.5 - all->coord.z) + all->res_y * (0.5 + all->coord.z) - all->spr.spriteheight * all->spr.tab[all->spr.n].size/* + all->draw.jump*/;
	all->spr.drawendy = all->spr.spriteheight * (0.5 + all->coord.z)+ all->res_y * (0.5 + all->coord.z) - all->spr.spriteheight * all->spr.tab[all->spr.n].size/* + all->draw.jump*/;
...
{
	all->spr.coeffx = all->spr.tab[n].w - all->spr.tab[n].w * ((double)(z - all->spr.drawstartx) / (double)(all->spr.drawendx - all->spr.drawstartx));
	all->spr.coeffy = all->spr.tab[n].h * ((double)(y - all->spr.drawstarty) / (double)(all->spr.drawendy - all->spr.drawstarty));
...
void	check_transparent(t_cub *all, int *img_data, int z, int y)
{
	...
	if (all->spr.tab[all->spr.n].anim == 1 )
		tmp = all->spr.tab[all->spr.n].data[(int)(all->spr.coeffx + all->spr.millis + all->spr.coeffy * all->spr.tab[all->spr.n].w)];
	else
	{
		if (all->spr.tab[all->spr.n].sens == 1 && all->info.raydiry > 0)
			tmp = all->spr.tab[all->spr.n].data[(int)((all->spr.tab[all->spr.n].w - all->spr.coeffx - 1) + all->spr.coeffy * all->spr.tab[all->spr.n].w)];
		else if (all->spr.tab[all->spr.n].sens == 0 && all->info.raydiry < 0)
			tmp = all->spr.tab[all->spr.n].data[(int)((all->spr.tab[all->spr.n].w - all->spr.coeffx - 1) + all->spr.coeffy * all->spr.tab[all->spr.n].w)];
		else tmp = all->spr.tab[all->spr.n].data[(int)(all->spr.coeffx + all->spr.coeffy * all->spr.tab[all->spr.n].w)];
	}
	if (tmp == -16777216)
...
void	print_sprite(t_cub *all, int *img_data)
{
	...
	while (all->spr.n < all->spr.nbr)
	{
		...
		if (all->spr.tab[all->spr.n].spr == 4)
			check_colision(all, all->spr.tab[all->spr.n].x, all->spr.tab[all->spr.n].y);
		all->spr.n++;
	}
}
